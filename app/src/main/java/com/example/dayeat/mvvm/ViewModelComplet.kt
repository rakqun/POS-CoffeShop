...
class ViewModelComplet:ViewModel() {
    ...
    fun queryGetPenjualan(context: Context, id: Int){
            ...
            if (conn!= null){
                try {
                    val query = "SELECT SUM(JSNOTVOID.Qty) NotQty, SUM(JSNOTVOID.NetTotal) NotTotal, SUM(JSVOID.Qty) VQty, SUM(JSVOID.NetTotal) VTotal \n" +
                            ...
                            "WHERE JS.ClosingID = ? AND JS.VoidDateTime IS NULL \n"
    ...
    fun queryGetPayment(context: Context, id: Int){
        GlobalScope.launch(Dispatchers.IO + Job()) {
            val conn = Conect().connection(context)
            ...
            if (conn!= null){
                try {
                    val query = "SELECT SP.Method, SUM(NetTotal) Total FROM J_Sales S\n" +
                            ...
                            "WHERE S.ClosingID = ? AND S.VoidDateTime IS NULL\n" +
                            "GROUP BY SP.Method"
                    val pp = conn.prepareStatement(query)
                    pp.setInt(1, id)
                    val rs = pp.executeQuery()
    ...
    fun queryGetCategory(context: Context, id: Int){
        GlobalScope.launch(Dispatchers.IO + Job()) {
            val conn = Conect().connection(context)
    ...
    fun queryGetTimeSales(context: Context, id: Int){
        GlobalScope.launch(Dispatchers.IO + Job()) {
            val conn = Conect().connection(context)
            ...
            if (conn!= null){
                try {
                    val query = "SELECT RIGHT('0' + CAST(DATEPART(HOUR, ClosedDateTime) AS VARCHAR), 2) + ':00' Time, SUM(NetTotal) NetTotal  FROM J_Sales\n" +
                            "WHERE ClosingID = ? AND VoidDateTime IS NULL\n" +
    ...
    fun queryGetMenuName(context: Context, id: Int){
        GlobalScope.launch(Dispatchers.IO + Job()) {
            val conn = Conect().connection(context)
